...
impl FragmentTree {
    pub fn build_display_list(&self, builder: &mut crate::display_list::DisplayListBuilder) {
        ...
        match (&self.canvas_background.style, self.root_fragments.first()) {
            (Some(canvas_background_style), Some(root_element_fragment)) => {
                let mut build_canvas_background = |root_element_fragment| {
                    builder.canvas_background(
                        &canvas_background_style,
                        root_element_fragment,
                        &self.initial_containing_block,
                        &self.scrollable_overflow,
                    )
                };
                match &*root_element_fragment.borrow() {
                    Fragment::Box(f) => build_canvas_background(f),
                    Fragment::AbsoluteOrFixedPositioned(f) => {
                        if let Some(hoisted) = &*f.hoisted_fragment.borrow() {
                            if let Fragment::Box(f) = &*hoisted.borrow() {
                                build_canvas_background(f)
                            } else {
                                debug_panic!("Expected a box-generated fragment")
                            }
                        } else {
                            debug_panic!("Missing hoisted fragment")
                        }
                    },
            ...
            (None, Some(_)) => {
                // Canvas background taken from `<body>` which has `display: none`
            },
            (Some(_), None) => debug_panic!("CanvasBackground::for_root_element is buggy"),
        }
...
fn iter_child_nodes<'dom, Node>(parent: Node) -> impl Iterator<Item = Node>
where
    Node: NodeExt<'dom>,
{
